...
func insertPendingMedia(t *testing.T, env *summaryEnv, albumID uuid.UUID, uploader []byte, createdAt time.Time, withThumb bool) *model.Media {
	...
	if _, err := env.pool.Exec(context.Background(), `
		INSERT INTO media (
			...
			media_type, confirmed, created_at, reservation_id
		) VALUES ($1,$2,$3,$4,$5,$6,$7,0,1,$8,'photo',FALSE,$9,$10)`,
		id, albumID, uploader, storageKey, thumbKey, make([]byte, 12),
		make([]byte, 48), make([]byte, 32), createdAt, reservationID,
	); err != nil {
...
func TestQueuePendingCleanup_RequiresUploaderAndPersistsKeys(t *testing.T) {
	...
	foreign, err := env.media.QueuePendingCleanup(ctx, pending.ID, album.ID, []byte("another-member"), pending.ReservationID)
	if err != nil {
	...
	batch, err := env.media.QueuePendingCleanup(ctx, pending.ID, album.ID, token, pending.ReservationID)
	if err != nil {
...
func TestRescheduleObjectCleanupKey_BacksOffPastTheSweepInterval(t *testing.T) {
	...
	if _, err := env.media.QueuePendingCleanup(ctx, pending.ID, album.ID, token, pending.ReservationID); err != nil {
